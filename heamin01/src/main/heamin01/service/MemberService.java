...
public class MemberService {
    ...
    public ResponseEntity<?> login(HttpSession session,LoginRequestDTO dto){
        ...
        //아이디 검사
        //조회 아이디 없을 시 예외 발생
        Member member = memberRepository.findByEmail(dto.getEmail()).orElseThrow(()-> new NotFoundException("이메일 또는 비밀번호가 일치하지 않습니다."));
        ...
        if(passwordEncoder.matches(dto.getPassword(),member.getPassword())){
            ...
            return ResponseEntity.ok(mapper.toLoginDto(member));
        }else{
            //ResponseEntity.status(HttpStatus.UNAUTHORIZED).body
            log.info("로그인 실패");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                    new ErrorResponse(HttpStatus.BAD_REQUEST,"이메일 또는 비밀번호가 일치하지 않습니다.","..")
            );
        }
    }
