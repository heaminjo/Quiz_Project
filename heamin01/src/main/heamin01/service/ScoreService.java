...
public class ScoreService {
    ...
    public ResponseEntity<?> insertScore(ScoreRequestDTO dto) {
        ...
        Category category = categoryRepository.findById(dto.getCategoryId()).orElseThrow(()->new NotFoundException("존재하지 않는 카테고리 입니다."));
                ...
                scoreRepository.save(new Score(member,category,dto.getResultNum()));
                //성공 응답 객체 반환
                return ResponseEntity.ok(new ApiResponse<ScoreResponseDTO>(true,"결과 등록에 성공하였습니다."));
        }
            ...
            public ResponseEntity<?> userBestScore(HttpSession session){
                ...
                //회원 존재 검사
               if(memberRepository.existsById((Long)session.getAttribute("loginID"))) throw new NotFoundException("존재하지 않는 회원입니다.");
